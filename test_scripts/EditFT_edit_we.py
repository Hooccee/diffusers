...
# #clip
# def find_common_subsequences2(target_tokens, source_tokens):
#     def truncate_at_eos(tokens):
#         if '<|endoftext|>' in tokens:
#             return tokens[:tokens.index('<|endoftext|>')]
#         return tokens
...
#     # 获取 text_ids
#     target_text_inputs= pipeline.tokenizer(
#         target_prompt,
#         padding="max_length",
#         max_length=77,
#         truncation=True,
...
#     source_text_inputs = pipeline.tokenizer(
#         source_prompt,
#         padding="max_length",
#         max_length=77,
#         truncation=True,
...
#     target_prompt_embeds = pipeline.text_encoder(target_text_ids.to(pipeline._execution_device), output_hidden_states=False)
#     source_prompt_embeds = pipeline.text_encoder(source_text_ids.to(pipeline._execution_device), output_hidden_states=False)



#     # 将 target_prompt 与 source_prompt 转换为 token 列表
#     target_tokens = pipeline.tokenizer.convert_ids_to_tokens(target_text_ids[0])
#     source_tokens = pipeline.tokenizer.convert_ids_to_tokens(source_text_ids[0])

#     # 找到所有公共子序列
#     common_subsequences = find_common_subsequences2(target_tokens, source_tokens)
