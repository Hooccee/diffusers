...
def interpolated_inversion(
    ...
    with pipeline.progress_bar(total=len(timesteps)-1) as progress_bar:
        for i, (t_curr, t_prev) in enumerate(zip(timesteps[:-1], timesteps[1:])):
            ...
            packed_latents_quarter = packed_latents + ((t_prev - t_curr)/4) * pred + 0.5*((t_prev - t_curr)/4) ** 2 * first_order

            # 计算四分之一点速度
            t_vec_quarter = torch.full((packed_latents.shape[0],), (t_curr + (t_prev - t_curr) / 4), dtype=packed_latents.dtype, device=packed_latents.device)
            joint_attention_kwargs['second_order'] = False
            #joint_attention_kwargs['third_order'] = True  # 标记为三阶计算
            pred_quarter, joint_attention_kwargs = pipeline.transformer(
                    hidden_states=packed_latents_quarter,
                    timestep=t_vec_quarter,
                    guidance=guidance_vec,
                    pooled_projections=pooled_prompt_embeds,
                    encoder_hidden_states=prompt_embeds,
                    txt_ids=text_ids,
            ...
            pred_quarter = pred_quarter[0]          

            # 计算二阶导数
            second_order = (pred_mid - 2 * pred_quarter + pred) / ((0.25*(t_prev - t_curr)) ** 2)   #在计算二阶导数时，差分可能引入较大的误差  

            # 防止精度问题
            packed_latents = packed_latents.to(torch.float32)
            pred = pred.to(torch.float32)
            pred_mid = pred_mid.to(torch.float32)
            pred_quarter = pred_quarter.to(torch.float32)
            # 更新潜变量
            packed_latents = packed_latents + (t_prev - t_curr) * pred + 0.5 * (t_prev - t_curr) ** 2 * first_order + (1/6) * (t_prev - t_curr) ** 3 * second_order
